...
export function BooksSection() {
    ...
    return (
        <Section title="📘 Books" color="green">
            <Button variant="light" color="green" onClick={async () => {
                show((await apiFetch("/books")).data);
            }}>
                GET /books
            </Button>
            ...
            <Modal opened={getIdModal} onClose={closeGetId} title="Get Book by ID" centered>
                <Stack>
                    ...
                    <Button color="green" onClick={async () => {
                        closeGetId();
                        show((await apiFetch(`/books/${getBookId}`)).data);
                    }}>Send</Button>
                </Stack>
            </Modal>
            ...
            <Modal opened={createModal} onClose={closeCreate} title="Create Book" centered>
                <Stack>
                    <TextInput label="Title" value={newBook.title} onChange={(e) => {
                        const v = e.target.value;
                        setNewBook((b) => ({...b, title: v}));
                    }}/>
                    <TextInput label="Author" value={newBook.author} onChange={(e) => {
                        const v = e.target.value;
                        setNewBook((b) => ({...b, author: v}));
                    }}/>
                    <NumberInput label="Year" value={newBook.year}
                                 onChange={(v) => setNewBook((b) => ({...b, year: Number(v)}))}/>
                    <TextInput label="ISBN" value={newBook.isbn} onChange={(e) => {
                        const v = e.target.value;
                        setNewBook((b) => ({...b, isbn: v}));
                    }}/>
                    <Button color="green" onClick={async () => {
                        closeCreate();
                        show((await apiFetch("/books", {
                            method: "POST",
            ...
            <Modal opened={updateModal} onClose={closeUpdate} title="Update Book" centered>
                <Stack>
                    <TextInput label="Book ID" value={updBook.id} onChange={(e) => {
                        const v = e.target.value;
                        setUpdBook((b) => ({...b, id: v}));
                    }}/>
                    <TextInput label="Title" value={updBook.title} onChange={(e) => {
                        const v = e.target.value;
                        setUpdBook((b) => ({...b, title: v}));
                    }}/>
                    <TextInput label="Author" value={updBook.author} onChange={(e) => {
                        const v = e.target.value;
                        setUpdBook((b) => ({...b, author: v}));
                    }}/>
                    <NumberInput label="Year" value={updBook.year}
                                 onChange={(v) => setUpdBook((b) => ({...b, year: Number(v)}))}/>
                    <TextInput label="ISBN" value={updBook.isbn} onChange={(e) => {
                        const v = e.target.value;
                        setUpdBook((b) => ({...b, isbn: v}));
                    }}/>
                    <TextInput label='Available ("true" / "false")' value={updBook.available} onChange={(e) => {
                        const v = e.target.value;
                        setUpdBook((b) => ({...b, available: v}));
                    }}/>
                    <Button color="green" onClick={async () => {
                        const {id, available, ...rest} = updBook;
                        closeUpdate();
                        show((await apiFetch(`/books/${id}`, {
                            method: "PUT",
                            headers: {"Content-Type": "application/json"},
                            body: JSON.stringify({...rest, available: available === "true"})
                        })).data);
                    }}>Update</Button>
                </Stack>
            </Modal>
